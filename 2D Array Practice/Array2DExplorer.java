...
public class Array2DExplorer {
    ...
    public int[] allRowSums(int[][] data) {
        return Arrays.stream(data).mapToInt(x -> Arrays.stream(x).sum()).toArray();
    }
    ...
    public int smallEven(int[][] matrix) {
        return Arrays.stream(matrix).flatMapToInt(x -> Arrays.stream(x)).filter(x -> x % 2 == 0).min().orElse(-1);
    }
